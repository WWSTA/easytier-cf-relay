# EasyTier Cloudflare Relay V1.2.0
...
由于作者不太会Workers的开发，所以使用了国模**GLM5.3**进行全栈开发。虽然与国模经历来回扯皮，但是可能仍然有一些不影响使用的特性，下个版本再修，欢迎fork修理。（其实是又没token了。。。总花费 61+15 RMB）
...
- **KV 审计记录**：网络分组 / 节点在线 / 路由信息 / 全局互联 / 连接列表 / 摘要注册六类事件
  ...
  管理员登录（IP/时间）、操作、查看为**硬设置**（wrangler.toml `ADMIN_AUDIT`，管理页不可关闭）
...
| [技术文档](docs/技术文档.md) | 协议实现详解、幽灵节点六重防线、KV 审计与黑名单设计、与官方对比、Hibernation 设计、测试体系 |
